...
public class UserNotFoundExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public final ResponseEntity<ErrorBean> handleAllExceptions(Exception e, WebRequest request) throws Exception {
		ErrorBean errorDetails = new ErrorBean(LocalDateTime.now(), e.getMessage(), request.getDescription(false));
		return new ResponseEntity<ErrorBean>(errorDetails, HttpStatus.INTERNAL_SERVER_ERROR);
	}

	@ExceptionHandler(UserNotFoundException.class)
	public final ResponseEntity<ErrorBean> handleUserNotFoundException(Exception e, WebRequest request) throws Exception {
		ErrorBean errorDetails = new ErrorBean(LocalDateTime.now(), e.getMessage(), request.getDescription(false));

...
type CreateConfig = (
	input: Pick<HandleInLibraryOptions, "userID" | "objectID" | "inLibrary">,
) => HandleInLibraryOptions;

const createSongConfig: CreateConfig = ({ userID, objectID, inLibrary }) => ({
	userID,
	objectID,
	inLibrary,
	typeName: "Song",
...
const createArtistConfig: CreateConfig = ({ userID, objectID, inLibrary }) => ({
	userID,
	objectID,
	inLibrary,
	typeName: "Artist",
...
const createPlaylistConfig: CreateConfig = ({ userID, objectID, inLibrary }) => ({
	userID,
	objectID,
	inLibrary,
	typeName: "Playlist",
...
export const addSongToLibrary = resolver<Song | null, SongID>(({ args, context }) =>
...
export const removeSongFromLibrary = resolver<Song | null, SongID>(({ args, context }) =>
	handleInLibrary(context.pg)(
		createSongConfig({
			inLibrary: false,
			objectID: args.songID,
...
export const addArtistToLibrary = resolver<Artist | null, ArtistID>(({ args, context }) =>
	handleInLibrary(context.pg)(
		createArtistConfig({
			inLibrary: true,
...
export const removeArtistFromLibrary = resolver<Artist | null, ArtistID>(({ args, context }) =>
	handleInLibrary(context.pg)(
		createArtistConfig({
			inLibrary: false,
			objectID: args.artistID,
...
export const addPlaylistToLibrary = resolver<Playlist | null, PlaylistID>(
	async ({ args, context }) => {
...
export const removePlaylistFromLibrary = resolver<Playlist | null, PlaylistID>(
	async ({ args, context }) => {
		const { playlistID } = args;
...
export const addAlbumToLibrary = resolver<Album | null, AlbumID>(async ({ args, context }) => {
	const { albumID } = args;
	...
	const songs = await query(context.pg)(SELECT_ALBUM_SONGS)({
...
export const removeAlbumFromLibrary = resolver<Album | null, AlbumID>(async ({ args, context }) => {
	const { albumID } = args;

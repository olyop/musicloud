...
interface GetAlbumSongsOptions<T> extends AlbumID, GetObjectsOptions<T> {}
...
interface GetUserAlbumPlaysOptions<T> extends UserID, AlbumID, GetObjectsOptions<T> {}
...
export const inLibrary = resolver(async ({ parent, context }) => {
	const { albumID } = parent;

	const albumSongs = await getAlbumSongs(context.pg)({
		albumID,
		columnNames: COLUMN_NAMES.SONG[0],
		parse: convertTableToCamelCase<Song>(),
	});

	const songsInLibrary = await Promise.all(
		albumSongs.map(({ songID }) =>

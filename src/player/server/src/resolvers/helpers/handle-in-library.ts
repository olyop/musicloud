...
import { INSERT_LIBRARY_OBJECT, EXISTS_LIBRARY_OBJECT, UPDATE_OBJECT_IN_LIBRARY } from "../../sql";

const checkObjectExists =
	(pg: PoolOrClient) =>
	async ({ objectID, typeName, tableName, columnName }: HandleInLibraryOptions) => {
		const doesObjectExist = await exists(pg)({
			value: objectID,
			table: tableName,
			column: columnName,
		});
...
const baseHandler = (client: PoolOrClient) => async (options: HandleInLibraryOptions) => {
	const { userID, objectID, inLibrary, columnName, libraryTableName, skipExists = false } = options;
	...
	const doesLibraryObjectExist = await query(client)(EXISTS_LIBRARY_OBJECT)({
		variables,

...
const isTokenExpiredError = (error: unknown) =>
	error instanceof TokenError && error.code === TokenError.codes.expired;
...
const determineAuthorization: DetermineAuthorization = async (pg, authorization) => {
...
const createContext = (): ApolloFastifyContextFunction<Context, CustomServer> => {

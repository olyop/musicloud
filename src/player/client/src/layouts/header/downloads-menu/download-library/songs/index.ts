...
import { LibrarySongsOrderBy, LibrarySongsOrderByField, OrderByDirection } from "../../../../../types"
...
const downloadSongs =
	(client: ApolloClient<unknown>) =>
		async (setCurrentDownload: SetCurrentDownload, setStatus: SetStatus) => {
			const { data } =
				await client.query<GetSongsTotalData>({
			...
			if (data.getLibrary.songsTotal) {
				...
				for (let index = 0; index <= songsTotal; index += 1) {
					...
					const { data: { getLibrary: { songAtIndex } } } =
					...
					if (songAtIndex) {
						...
						const { songID, album: { albumID } } = songAtIndex

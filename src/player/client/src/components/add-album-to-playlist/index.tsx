...
const bem =
	createBEM("AddAlbumToPlaylist")

const AddAlbumToPlaylist: FC<PropTypes> = ({ albumID, onClose }) => {
	const [ playlistID, setPlaylistID ] = useState<string | null>(null)

	const { data: playlistsData } =
		useQuery<GetUserPlaylistsData, AlbumID>(GET_USER_PLAYLISTS, {
			variables: { albumID },
			fetchPolicy: "cache-and-network",
		})

	const [ add, { data: mutationData } ] =
		useMutation<AddToPlaylistData, AddToPlaylistVars>(ADD_ALBUM_TO_PLAYLIST)

...
const uploadSong =
	(client: PoolClient, s3: S3Client, ag: SearchIndex) => async (options: Options) => {
		const { body, song, albumID, album, albumCoverURL } = options;
		...
		try {
			const songResult = await query(client)(INSERT_SONG)({
			...
			for (const genreItem of songGenreList) {
				...
				const genreID = await getGenreID(client)(name);
			...
			await uploadFileToS3(s3)(determineCatalogAudioPath(songID), audio);

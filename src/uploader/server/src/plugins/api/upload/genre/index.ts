...
export const uploadGenre: FastifyPluginAsync =
	...
	async fastify => {
		fastify.post<Route>("/genre", { onRequest: fastify.authenticate }, async (request, reply) => {
			const { body } = request;
			const name = trim(body.name);

			const doesGenreAlreadyExist = await exists(fastify.pg.pool)({
				value: name,
				column: "name",
				table: "genres",
			});
			...
			try {
				...
				const result = await query(client)(INSERT_GENRE)({
